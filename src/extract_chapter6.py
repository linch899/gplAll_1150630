...
def parse_main_version(docx_path, gpl_map_exact, gpl_map_norm, comparison_log):
    ...
    chinese_nums = ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一"]
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        if txt == "決標階段" or txt.startswith("第六章") or (txt.startswith("章") and len(txt) < 15):
            continue
        ...
        if numPr is not None and ilvl_val == 1:
            if current_lines:
                ...
                current_lines = []
                start_idx = -1
            cleaned_txt = txt.replace("or", "或")
            current_sec = sec_names.get(cleaned_txt, f"第一節 {cleaned_txt}")
            continue

        if txt in sec_names or txt in sections_in_doc or txt.replace("or", "或") in sec_names:
            if current_lines:
                letters.append((start_idx, current_lines, current_sec))
                current_lines = []
                start_idx = -1
            cleaned_txt = txt.replace("or", "或")
            current_sec = sec_names.get(cleaned_txt, sec_names.get(sections_in_doc[0]))
            continue
    ...
    for para_idx, lines, sec_name in letters:
        ...
        if db_match:
            ...
            unmatched_count += 1
            placeholder_id = f"6-{unmatched_count:02d}"
            subject = lines[0]
...
def parse_new_version(docx_path, gpl_map_exact, gpl_map_norm, comparison_log):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        first = p_lines[0]
        if first.startswith("第六章") or first.startswith("章") or first == "決標階段":
            continue
        ...
        if first in sec_names or first in sections_in_doc or first.replace("or", "或") in sec_names:
            if current_lines:
                letters.append((start_idx, current_lines, current_sec))
                current_lines = []
                start_idx = -1
            cleaned_first = first.replace("or", "或")
            current_sec = sec_names.get(cleaned_first, sec_names.get(sections_in_doc[0]))
            continue
    ...
    for para_idx, lines, sec_name in letters:
        ...
        if db_match:
            ...
            unmatched_count += 1
            placeholder_id = f"6-new-{unmatched_count:02d}"
            content = "\n".join(lines)
...
def main():
    ...
    docx_path_main = os.path.join(base_dir, "第六章", "第六章_組長版.docx")
    docx_path_new = os.path.join(base_dir, "第六章", "第六章(112.1開始增加).docx")
    json_path = os.path.join(base_dir, "gplAll_1150630.json")
    
    output_json_path = os.path.join(base_dir, "第六章", "第六章.json")
    output_log_path = os.path.join(base_dir, "第六章", "第六章比對紀錄.txt")
    ...
    print("Parsing main chapter 6 docx...")
    ...
    print("Parsing new 112.1 chapter 6 docx...")
    ...
    print(f"Saving combined sorted Chapter 6 database to {output_json_path}...")
    ...
    main_matched_count = sum(1 for item in comparison_log if item["來源文件"] == "第六章_組長版.docx" and item["比對結果"] == "匹配成功")
    new_matched_count = sum(1 for item in comparison_log if item["來源文件"] == "第六章(112.1開始增加).docx" and item["比對結果"] == "匹配成功")

    with open(output_log_path, "w", encoding="utf-8") as f:
        f.write("=========================================================================\n")
        f.write("                       第六章 解釋函令比對紀錄檔                         \n")
        f.write("=========================================================================\n\n")
        f.write(f"統計摘要：\n")
        f.write(f"  - 主檔版 (第六章_組長版.docx)：已匹配 {main_matched_count} 筆，未匹配 {unmatched_main} 筆\n")
        f.write(f"  - 新增版 (第六章(112.1開始增加).docx)：已匹配 {new_matched_count} 筆，未匹配 {unmatched_new} 筆\n")
    ...
    print("Chapter 6 Word to JSON processing completed successfully.")
