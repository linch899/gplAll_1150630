...
def parse_main_version(docx_path, gpl_map_exact, gpl_map_norm, comparison_log):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        if txt == "履約管理階段" or txt.startswith("第七章") or (txt.startswith("章") and len(txt) < 15):
            continue
    ...
    for para_idx, lines, sec_name in letters:
        ...
        if db_match:
            ...
            unmatched_count += 1
            placeholder_id = f"7-{unmatched_count:02d}"
            subject = lines[0]
...
def parse_new_version(docx_path, gpl_map_exact, gpl_map_norm, comparison_log):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        first = p_lines[0]
        if first.startswith("第七章") or first.startswith("章") or first == "履約管理階段":
            continue
...
def main():
    ...
    docx_path_main = os.path.join(base_dir, "第七章", "第七章_組長版.docx")
    docx_path_new = os.path.join(base_dir, "第七章", "第七章(112.1開始增加).docx")
    json_path = os.path.join(base_dir, "gplAll_1150630.json")
    
    output_json_path = os.path.join(base_dir, "第七章", "第七章.json")
    output_log_path = os.path.join(base_dir, "第七章", "第七章比對紀錄.txt")
    ...
    print(f"Saving combined sorted Chapter 7 database to {output_json_path}...")
    ...
    main_matched_count = sum(1 for item in comparison_log if item["來源文件"] == "第七章_組長版.docx" and item["比對結果"] == "匹配成功")
    new_matched_count = sum(1 for item in comparison_log if item["來源文件"] == "第七章(112.1開始增加).docx" and item["比對結果"] == "匹配成功")

    with open(output_log_path, "w", encoding="utf-8") as f:
        f.write("=========================================================================\n")
        f.write("                       第七章 解釋函令比對紀錄檔                         \n")
        f.write("=========================================================================\n\n")
        f.write(f"統計摘要：\n")
        f.write(f"  - 主檔版 (第七章_組長版.docx)：已匹配 {main_matched_count} 筆，未匹配 {unmatched_main} 筆\n")
        f.write(f"  - 新增版 (第七章(112.1開始增加).docx)：已匹配 {new_matched_count} 筆，未匹配 {unmatched_new} 筆\n")
    ...
    print("Chapter 7 Word to JSON processing completed successfully.")
