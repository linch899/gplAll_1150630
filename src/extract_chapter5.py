...
def parse_main_version(docx_path, gpl_map_exact, gpl_map_norm, comparison_log):
    ...
    sections = ["開標作業", "不予開標", "審標作業", "辦理評選作業或審查作業", "最低標標價偏低", "其他開標階段事項"]
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        if txt == "開標審標階段" or txt.startswith("第五章") or (txt.startswith("章") and len(txt) < 15):
            continue
        ...
        if txt in sec_names or txt in sections:
            if current_lines:
                letters.append((start_idx, current_lines, current_sec))
                current_lines = []
                start_idx = -1
            current_sec = sec_names.get(txt, sec_names.get(sections[0]))
            continue
    ...
    for para_idx, lines, sec_name in letters:
        ...
        if db_match:
            ...
            unmatched_count += 1
            placeholder_id = f"5-{unmatched_count:02d}"
            subject = lines[0]
...
def parse_new_version(docx_path, gpl_map_exact, gpl_map_norm, comparison_log):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        first = p_lines[0]
        if first.startswith("第五章") or first.startswith("章") or first == "開標審標階段":
            continue
        ...
        if first in sec_names or first in sections:
            if current_lines:
                letters.append((start_idx, current_lines, current_sec))
                current_lines = []
                start_idx = -1
            current_sec = sec_names.get(first, sec_names.get(sections[0]))
            continue
    ...
    for para_idx, lines, sec_name in letters:
        ...
        if db_match:
            ...
            unmatched_count += 1
            placeholder_id = f"5-new-{unmatched_count:02d}"
            content = "\n".join(lines)
...
def main():
    ...
    docx_path_main = os.path.join(base_dir, "第五章", "第五章.docx")
    docx_path_new = os.path.join(base_dir, "第五章", "第五章(112.1開始增加).docx")
    ...
    print("Parsing main chapter 5 docx...")
    ...
    print("Parsing new 112.1 chapter 5 docx...")
    ...
    main_matched_count = sum(1 for item in comparison_log if item["來源文件"] == "第五章.docx" and item["比對結果"] == "匹配成功")
    new_matched_count = sum(1 for item in comparison_log if item["來源文件"] == "第五章(112.1開始增加).docx" and item["比對結果"] == "匹配成功")

    with open(output_log_path, "w", encoding="utf-8") as f:
        f.write("=========================================================================\n")
        f.write("                       第五章 解釋函令比對紀錄檔                         \n")
        f.write("=========================================================================\n\n")
        f.write(f"統計摘要：\n")
        f.write(f"  - 主檔版 (第五章.docx)：已匹配 {main_matched_count} 筆，未匹配 {unmatched_main} 筆\n")
        f.write(f"  - 新增版 (第五章(112.1開始增加).docx)：已匹配 {new_matched_count} 筆，未匹配 {unmatched_new} 筆\n")
    ...
    print("Chapter 5 Word to JSON processing completed successfully.")
